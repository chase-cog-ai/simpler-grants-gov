...
test("happy path apply workflow - Organization User (SF424B and SF-LLL)", async ({
  ...
  try {
    ...
    console.log("  ✓ Application name entered");
  } catch (err) {
    const modalHtml = await modal.innerHTML();
    console.log("[DEBUG] Modal HTML for application name:\n", modalHtml);
    const allLabels = await modal.locator('label').allTextContents();
    console.log("[DEBUG] Modal labels:", allLabels);
    ...
    for (let i = 0; i < allInputs.length; i++) {
      ...
      console.log(`[DEBUG] Input #${i} type: ${inputType}, placeholder: ${inputPlaceholder}`);
    }
    throw new Error("Could not fill 'Name of this application'");
  }
  ...
  if (await sf424bLink.count() > 0) {
    ...
    if (noLabelCount > 0) {
      ...
      await expect(noLabelLocator.first()).toBeVisible({ timeout: 3000 });
      ...
      if (await fallbackLabel.count() > 0) {
        ...
        await expect(fallbackLabel.first()).toBeVisible({ timeout: 3000 });
